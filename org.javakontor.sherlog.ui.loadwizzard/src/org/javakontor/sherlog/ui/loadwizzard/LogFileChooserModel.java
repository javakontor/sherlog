...
public class LogFileChooserModel extends AbstractModel<LogFileChooserModel, DefaultReasonForChange> {
  ...
  public ValidationResult validateForm() {
    ...
    if (this._selectedFlavour == null) {
      return new ValidationResult(LoadLogFileWizzardMessages.noFlavourSelected);
    }
  ...
  public ValidationResult validateSelectedLogFile() {
    ...
    if (file == null) {
      return new ValidationResult(LoadLogFileWizzardMessages.noFileSelected);
    }
    if (!file.isFile()) {
      return new ValidationResult(format(LoadLogFileWizzardMessages.selectedFileIsNotAFile, file.getAbsolutePath()));
    }
