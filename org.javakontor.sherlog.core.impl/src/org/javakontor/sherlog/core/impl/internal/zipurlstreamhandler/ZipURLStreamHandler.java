...
/**
 * <p>
 * A URLStreamHandler which allows accessing specific zip entries within a zip
 * file. The format of the file part of the URL is
 * <code>[full path to the zip file]|[name of zip entry]</code>, but you are
 * encouraged to use the
 * <code>createURL(String fileName, String entryName)</code> method instead of
 * creating a URL manually. The host and port part of the URL should be
 * <code>null</code>. An application is responsible for setting a
...
public class ZipURLStreamHandler extends AbstractURLStreamHandlerService {
	...
	public static final Object URL_HANDLER_PROTOCOL =
		new String[] {"zip"};
	...
	protected void parseURL(URL url, String spec, int start, int limit) {
		...
		String fileName = barIndex == -1 ? urlFile : urlFile.substring(0,
				barIndex);
		String entryName = barIndex == -1 ? "" : urlFile
				.substring(barIndex + 1);

		int lastSlashIndex = entryName.lastIndexOf("/");
		String newEntryName = entryName.substring(0, lastSlashIndex + 1)
				+ spec.substring(start, limit);

		setURL(url, url.getProtocol(), "", -1, fileName + "|" + newEntryName,
				null);
	}
	 ...
	 * name. To use the resulting <code>URL</code>, an application must set a
	 ...
	 * protocol. Returns <code>null</code> if the "zip: protocol is
	 * unrecognized.
	 */
	 ...
	 * <code>URLStreamHandlerFactory</code> (via the <code>URL</code> class)
	 * which will return a <code>ZipURLStreamHandler</code> for the protocol
	 * name given to this method. Returns <code>null</code> if the specified
	 * protocol is unrecognized.
	 */

	public static URL createURL(String protocol, String fileName,
			String entryName) {
		try {
