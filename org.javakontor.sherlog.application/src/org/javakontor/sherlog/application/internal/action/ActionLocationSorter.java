 ...
 */
public class ActionLocationSorter {

  public List<ActionLocation> sort(List<ActionLocation> unsortedLocations) {
    ...
    final Map<String, ActionLocation> _locationMap = new Hashtable<String, ActionLocation>();

    // Holds list of Location that cannot be ordered (yet)
    List<ActionLocation> retryList = new ArrayList<ActionLocation>();

    for (ActionLocation unsortedLocation : unsortedLocations) {

      _locationMap.put(unsortedLocation.getId(), unsortedLocation);
    ...
    while (!done) {
      ...
      List<ActionLocation> curRetry = new ArrayList<ActionLocation>(retryList);
      int retryCount = retryList.size();
      retryList.clear();

      // Walk the current list seeing if any entries can now be resolved
      for (Iterator<ActionLocation> iterator = curRetry.iterator(); iterator.hasNext();) {
        ActionLocation location = iterator.next();
    ...
    final List<ActionLocation> result = new LinkedList<ActionLocation>();
  ...
  private boolean processAdditions(List<String> allItems, ActionLocation unsortedLocation) {
    final int idx = getInsertionIndex(allItems, unsortedLocation);
    if (idx == -1) {
      return false; // can't process (yet)
    }

    allItems.add(idx, unsortedLocation.getId());
    return true;
  }
   ...
   * @return
   */
  private int getInsertionIndex(List<String> items, ActionLocation location) {
  ...
  public static void main(String[] args) {
    try {
      ...
      ActionLocationSorter sorter = new ActionLocationSorter();
